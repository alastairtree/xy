...
float xyMap(float encoded, vec2 map, vec2 meta, int mode, float constant) {
  // Linear axes stay centered on the encoded viewport midpoint (§4/§16):
  // subtract before multiplying so two large clip-space terms never cancel in
  // f32. Non-linear axes decode first because their transforms are not affine.
  if (mode == 0) return (encoded - map.y) * map.x;
